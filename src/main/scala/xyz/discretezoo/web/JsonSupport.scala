...
import xyz.discretezoo.web.db.MBGENMatrixWithCharacteristicS.MatrixWithCharacteristicS
import xyz.discretezoo.web.db.MBGENMatrixWithCharacteristicSView.MatrixWithCharacteristicSView
import xyz.discretezoo.web.db.MBGENMatrixS.MatrixS
...
trait JsonSupport extends SprayJsonSupport with DefaultJsonProtocol with NullOptions {
  ...
  implicit object formatZooObject extends RootJsonFormat[ZooObject] {
    override def write(o: ZooObject): JsValue = o match {
      case o: MatrixWithCharacteristicS => JsObject(
        List(
        Some("matrixID" -> o.matrixID.toJson),
        Some("positive_definite" -> o.positiveDefinite.toJson)
        ).flatten: _*
      )
case o: MatrixWithCharacteristicSView => JsObject(
        List(
        Some("positive_definite" -> o.positiveDefinite.toJson),
